...
@section('title', __('my/events/theses.create.title'))

@section('h1', __('my/events/theses.create.h1'))
...
@section('content')
	...
	x-data="{
        ...
        @if ($conference->sections->count() > 0)
            <div class="form__row">
                <label class="form__label">{{ __('my/events/theses.create.choose_section') }} (*)</label>
                <select name="section_id" data-scroll="500" data-class-modif="form" data-name="section_id">
                    <option value="" selected>{{ __('my/events/theses.create.choose_section') }}</option>
        ...
        <div class="form__row">
            <label class="form__label">{{ __('my/events/theses.create.report_form') }} (*)</label>
            <select name="report_form" data-scroll="500" data-class-modif="form" data-name="report_form">
				@foreach (Src\Domains\Conferences\Enums\ThesisReportForm::cases() as $reportForm)
					<option value="{{ $reportForm->value }}" selected>{{ $reportForm->toString() }}</option>
				@endforeach
            </select>
        </div>
		...
		<div class="form__row">
			<div class="checkbox">
				...
				<label for="solicited_talk" class="checkbox__label">
					<span class="checkbox__text">{{ __('my/events/theses.create.solicited_talk') }}</span>
				</label>
			</div>
			<div>{{ __('my/events/theses.create.solicited_talk_hint') }}</div>
		</div>

        <label class="form__label _mb0">{{ __('my/events/theses.create.authors_list') }} (*) </label>
		...
		<div x-data="{
					...
					<div class="author">
						...
						<div class="form__row">
							<div id="affiliations" class="form__row" x-data="{
								...
								<label class="form__label" for="f_1">{{ __('my/events/theses.create.affiliations') }}</label>
									...
									<div class="affiliation form__line" x-data="{
											...
											<div class="input-tips" x-show="show" x-transition.opacity>
												<ul>
													...
													<template x-if="suggestions.length === 0">
														<li>{{ __('my/events/theses.create.empty_search') }}</li>
													</template>
												</ul>
											</div>
										</div>
											...
											<div class="checkbox-items">
												<div class="checkbox">
													...
													<label :for="'a_1' + id + affId" class="checkbox__label">
														<span class="checkbox__text">{{ __('my/events/theses.create.has_mistake') }}</span>
													</label>
												</div>
												<div class="checkbox">
													...
													<label :for="'a_2' + id + affId" class="checkbox__label">
														<span class="checkbox__text">{{ __('my/events/theses.create.no_affiliation') }}</span>
													</label>
												</div>
											</div>
										</div>
										<div class="form__line">
											<button class="form__button button button_outline" type="button"
												@click="remove(affId)">
												{{ __('my/events/theses.create.remove_affiliation') }}
											</button>
										</div>
									</div>
								</template>
								...
								<div class="form__line">
									<button class="form__button button" type="button" @click="add">
										{{ __('my/events/theses.create.add_affiliation') }}
									</button>
								</div>
							</div>
						</div>
						...
						<div class="form__row">
							<button class="form__button button button_outline" type="button" @click="removeAuthor(id)">{{ __('my/events/theses.create.remove_author') }}</button>
						</div>
			...
			<div class="form__row" style="margin-top: 10px">
				<button class="form__button button" type="button" @click="add()">{{ __('my/events/theses.create.add_author') }}</button>
			</div>
		</div>


        <div class="form__row" id="reporter">
            <label class="form__label">{{ __('my/events/theses.create.reporter') }} (*)</label>
            ...
            <div class="checkbox">
                ...
                <label for="d_1" class="checkbox__label">
                    <span class="checkbox__text">{{ __('my/events/theses.create.young') }}</span>
                </label>
            </div>
        </div>

        <div class="form__row" id="contact">
            <label class="form__label">{{ __('my/events/theses.create.contact') }} (*)</label>
        ...
        <div class="form__row">
            <label class="form__label" for="c_1">{{ __('my/events/theses.create.contact_email') }} (*)</label>
		...
		@if ($conference->thesis_instruction)
			<div class="form__row">
				<label class="form__label">{{ __('my/events/theses.create.instruction') }}</label>
		...
		<div class="form__row editor-title" :class="form.invalid('title') && '_error'" x-data="{
            ...
            <label class="form__label" for="n_1">{{ __('my/events/theses.create.report_title') }} (*)</label>
			<style>
		...
		x-data="{
            ...
            <label class="form__label" for="t_1">{{ __('my/events/theses.create.report_text') }} (*)</label>
			<div class="form__line">
				<style>
					.ck-content {
						height: 300px;
			...
			<div class="form__line">
				{{ __('my/events/theses.create.simbols') }}:
        ...
        <div class="form__row">
            <div class="form__btns">
				...
				>
					{{ __('my/events/theses.create.preview') }}
        ...
        <div class="form__row">
            <div class="form__btns">
                <button class="form__button button button_primary" :disabled="form.processing" type="submit">
					{{ __('my/events/theses.create.send_abstracts') }}
					<x-loader class="tw-w-5 tw-h-4"/>
				</button>
            </div>
        </div>
		...
		<template x-if="form.hasErrors">
			<div class="form__row">
				<div class="form__error">{{ __('my/events/theses.create.mistakes_in_form') }}:</div>
				<ul>
